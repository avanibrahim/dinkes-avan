// src/pages/Diagnosis/Hiv.tsx
import React, { useState, useRef } from "react";
import RightPanel from "@/components/RightPanel";
import { Bell, Settings, HeartPulse, RefreshCw } from "lucide-react";
...
const gejalaList = [
  { id: "demam_berulang", pertanyaan: "Apakah Anda mengalami demam berulang atau mirip flu yang tidak kunjung reda?" },
  { id: "kelelahan_ekstrem", pertanyaan: "Apakah Anda merasakan kelelahan ekstrem tanpa sebab yang jelas?" },
  { id: "berat_badan_turun", pertanyaan: "Apakah berat badan turun drastis akhir-akhir ini?" },
  { id: "diare_berkepanjangan", pertanyaan: "Apakah Anda mengalami diare terus-menerus atau kronis?" },
...
export default function HivDiagnosis() {
  ...
  const handleSubmit = () => {
    setLoading(true);
    // show loading overlay for 5s, then call backend
    setTimeout(async () => {
      try {
  ...
  const exportPDF = () => {
    if (pdfRef.current) {
      html2pdf().from(pdfRef.current).save("hasil-diagnosis-hiv.pdf");
    }
  };

  const getStatusColor = (status: string) => {
    switch (status.toLowerCase()) {
      case "positif": return "text-red-600 bg-red-100";
      case "netral":  return "text-yellow-600 bg-yellow-100";
      ...
      <motion.main
        ...
        >
          ...
          <motion.div className="bg-white rounded-2xl shadow-lg p-8 relative" variants={fadeUp}>
              ...
              <>
                ...
                {result && (
                  <motion.div ref={pdfRef} className="space-y-6" variants={fadeUp}>
                    {/* Status */}
                    <div className={`p-6 rounded-lg text-center font-bold text-xl ${getStatusColor(result.status)}`}>
                      ANDA {result.status.toUpperCase()} HIV/AIDS
                    </div>
                    {/* Confidence */}
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={result.detail} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                          ...
                          <Bar dataKey="cf_hasil" fill="#4ade80">
                            <LabelList dataKey="cf_hasil" formatter={val => `${(val * 100).toFixed(0)}%`} position="top" />
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                    ...
                    <motion.div variants={fadeUp}>
                      ...
                      <ul className="list-disc list-inside text-sm text-gray-800 space-y-1">
                        {result.catatan.map((c: string, i: number) => <li key={i}>{c}</li>)}
                      </ul>
                    </motion.div>
                    {/* Export */}
                    <motion.button onClick={exportPDF} className="w-full bg-gradient-to-r from-teal-400 to-cyan-500 text-white py-2 rounded-lg font-medium" whileHover={{ scale: 1.02 }} variants={fadeUp}>
                      Export ke PDF
                    </motion.button>
                  </motion.div>
                )}
            ...
            {/* Loading Overlay */}
            {loading && (
              <motion.div className="fixed inset-0 bg-white bg-opacity-90 flex flex-col items-center justify-center z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <HeartPulse className="w-16 h-16 text-teal-500 animate-pulse mb-4" />
                <p className="text-gray-700 font-medium">Mohon tunggu, sistem sedang menganalisis gejala...</p>
              </motion.div>
            )}
          </motion.div>
        </motion.section>
