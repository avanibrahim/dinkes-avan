...
const gejalaList = [
  {
    ...
    pertanyaan:
      "Apakah Anda mengalami batuk lebih dari 2 minggu, berdahak atau tidak?",
  },
  {
  ...
  },
  {
    id: "demam_berulang",
    pertanyaan:
      "Apakah Anda mengalami demam meriang yang hilang-timbul tanpa sebab?",
  },
  {
    id: "berat_badan_turun",
    pertanyaan:
      "Apakah berat badan Anda turun atau tidak naik dalam 2 bulan terakhir?",
  },
  {
    id: "nafsu_makan_menurun",
    pertanyaan:
      "Apakah Anda merasakan penurunan nafsu makan yang signifikan?",
  },
  {
    id: "lemah_letih_lesu",
    pertanyaan:
      "Apakah Anda merasa lemah, letih, dan lesu setiap hari?",
  },
];
...
export default function Tbc() {
  ...
  const exportPDF = () => {
    if (pdfRef.current) html2pdf().from(pdfRef.current).save("hasil-diagnosis-tbc.pdf");
  };
                ...
                {result && (
                  <motion.div ref={pdfRef} className="space-y-6" variants={fadeUp}>
                    <div className={`p-6 rounded-lg text-center font-bold text-xl ${getStatusColor(result.status)}`}>
                      ANDA {result.status.toUpperCase()} TBC
                    </div>
